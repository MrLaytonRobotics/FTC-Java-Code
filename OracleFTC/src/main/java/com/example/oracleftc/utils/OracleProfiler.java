...
import com.example.oracleftc.hardware.OracleHardware;
import com.example.oracleftc.hardware.OracleLynxCommandListener;
...
public class OracleProfiler implements OpModeManagerImpl.Notifications, OracleLynxCommandListener {
    private static final OracleProfiler instance = new OracleProfiler();
    ...
    {
        instance.attached= OracleHardware.addLynxCommandListener(instance);
    ...
    private synchronized void logCommand(LynxCommand lynxCommand) {
        ...
        {
            try {
                if(lynxCommand instanceof LynxGetADCCommand)
                {
                    LynxGetADCCommand command = (LynxGetADCCommand)lynxCommand;
                    instance.logFileWriter.write(String.format(Locale.ENGLISH, "%d ADC-COMMAND %d\n", System.nanoTime(), (byte)ReflectionUtils.getFieldValue(command, "channel")));

                }else {
                    instance.logFileWriter.write(String.format(Locale.ENGLISH, "%d COMMAND %s\n", System.nanoTime(), lynxCommand.toString()));
    ...
    private synchronized void logResponseReceived(LynxMessage response, LynxCommand respondedCommand)
    {
        ...
        {
            try {
                instance.logFileWriter.write(String.format(Locale.ENGLISH, "%d RESPONSE %s %s\n", System.nanoTime(), respondedCommand.toString(), response.toString()));
            } catch (IOException e) {
