...
public class OracleLastKnown<T> {
    ...
    public OracleLastKnown()
    {
        this(500, true);
    }
    public OracleLastKnown(boolean canExpire)
    {
        this(0,canExpire);
    }
    public OracleLastKnown(double msFreshness, boolean canExpire)
    {
        this.value       = null;
        this.isValid     = false;
    ...
    public static <X> OracleLastKnown<X>[] createArray(int length)
    {
        OracleLastKnown<X>[] result = new OracleLastKnown[length];
        for (int i = 0; i < length; i++)
            result[i] = new OracleLastKnown<X>();
        return result;
    }

    public static <X> void invalidateArray(OracleLastKnown<X>[] array)
    {
