...
import orgoracleftc.utils.ReflectionUtils;
...
public class OracleLynxModule extends LynxModule {
    private static final String TAG = "PhotonLynxModule";

    /**
     * Create a PhotonLynxModule from a standard LynxModule
     */
    public static OracleLynxModule fromLynxModule(LynxModule lynxModule) {
        try {
            OracleLynxModule module = new OracleLynxModule(
                    lynxModule,
    ...
    private OracleLynxModule(LynxModule delegate, LynxUsbDevice lynxUsbDevice, Integer moduleAddress, Boolean isParent, Boolean isUserModule) {
                ...
                if (pair != null)
                {
                    ...
                    if (incomingMessage.isAck() || incomingMessage.isNack())
                    {
                        ...
                        if (ackdCommand != null)
                        {
                            ...
                            for (OracleLynxCommandListener listener:
                                    listeners) {
    ...
    public void sendCommand(LynxMessage command) throws InterruptedException, LynxUnsupportedCommandException {
        ...
        if(command instanceof LynxCommand)
        {
            synchronized (this)
            {
                while(unfinishedCommands.size()>OracleHardware.optimize.maximumParallelCommands());
    ...
    private List<OracleLynxCommandListener> listeners = new ArrayList<OracleLynxCommandListener>();
     ...
     * @return Success
     */
    public boolean addLynxCommandListener(OracleLynxCommandListener listener)
    {
