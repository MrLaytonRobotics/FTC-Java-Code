package com.example.oracleftc.math

import com.example.oracleftc.math.Pose2d.Companion.exp
import com.example.oracleftc.math.Rotation2d.Companion.exp
 ...
 * Vector \((x, y)\)
 */
data class Vector2d(@JvmField open val x: Double, @JvmField val y: Double) {
...
data class PoseVelocity2d(@JvmField val linearVel: Vector2d, @JvmField val angVel: Double) {
    ...
    operator fun unaryMinus() = PoseVelocity2d(linearVel.unaryMinus(), -angVel)
}
...
fun compute(
    ...
    return targetVelTarget +
            ...
            PoseVelocity2d(
                Vector2d(
                     velErrorActual.linearVel.x,
                     velErrorActual.linearVel.y,
                ),
                 velErrorActual.angVel,
            )
}